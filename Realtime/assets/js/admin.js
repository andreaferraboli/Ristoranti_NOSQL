...
function eliminaRistorante(event) {

    // alert("elimina: " +event.target.id);
    // alert("ciao")
    // let restaurantRef = ref(db, "Ristoranti" + "/" + "0");
    //
    // // remove review
...
onValue(dbRef, (snap) => {
    ...
    for (let i = 0, l = Object.keys(obj).length; i < l; i++) {
        ...
        stringBuilder += '</div>'
        stringBuilder += '<div class="card-item">'
        stringBuilder += '<button id="' + i + '" onclick="eliminaRistorante()" class="btn-delete" data-toggle="tooltip" data-placement="top" title="elimina">Elimina</button>'
        stringBuilder += '<button class="btn-modify" data-toggle="tooltip" data-placement="top" title="modifica">Modifica</button>'
        stringBuilder += '</div>'
        stringBuilder += '</div>'
        stringBuilder += '</div>'
        stringBuilder += '</div>'
